...
char msg[42];
char str[42 * (1 << 12)], tmp[42 * (1 << 12)], buf[42 * (1 << 12) + 42];
...
void send_all(int fd, char *str_req){
  ...
  while(temp){
    if (temp->fd != fd && FD_ISSET(temp->fd, &cpy_write)){
      if (send(temp->fd, str_req, strlen(str_req), 0) < 0) fatal();
    }
  }
}
...
int rm_client(int fd){
  ...
  if (temp && temp->fd == fd){
    ...
    while (temp && temp->next && temp->next->fd != fd) temp = temp->next;
    del = temp->next;
...
int main(int ac, char **av){
  ...
  while(1){
    ...
    if (select(get_max_fd() + 1, &cpy_read, &cpy_write, NULL, NULL) < 0) continue;
